...
import { createWireframeGeometry, onMouseMove, addBezierCurve } from './hopscotch_functions.js';
...
fetch('hopscotch.json')
  ...
  .then(data => {
    ...
    for (let i = 0; i < indices.length / 3; i += 2) {
      materials.push(new THREE.MeshBasicMaterial({ color: Math.random() * 0xffffff , opacity: min_opacity, transparent: true}));
    }
    ...
    const wireframeMaterial = new THREE.LineBasicMaterial({ color: 0xffffff , linewidth:wire_width, opacity: wire_opacity, transparent: true });
